        ...
        feature: {
            ...
            dataView: {
                ...
                optionToContent: function (opt) {
                    ...
                    let tdHeads = '<td  style="padding: 0 10px;width:12%;">Time</td>'; //表头
                    let tdBodys = ''; //数据
                    series.forEach(function (item) {
                        //组装表头
                        tdHeads += `<td style="padding: 0 10px;width:12%;">${item.name}</td>`;
                    });
                    ...
                    for (let i = 0, l = axisData.length; i < l; i++) {
                        for (let j = 0; j < series.length; j++) {
                            //组装表数据
                            tdBodys += `<td style="width:12%;background-color:#d0f2fd;">${series[j].data[i]}</td>`;
                        }
                        table += `<tr><td style="padding: 0 10px;width:12%;background-color:#d0f2fd;">${axisData[i]}</td>${tdBodys}</tr>`;
                        tdBodys = '';
                    }
                    table += '</tbody></table>';
                    return table;
    ...
    xAxis: [
        {
            type: 'category',
            boundaryGap: false,
            data: ['WW01', 'WW02', 'WW03', 'WW04', 'WW05', 'WW06', 'WW07'],
            axisLabel: {
                show: true,
                textStyle: {
                    fontFamily: "Consolas"
...
var lineChart_size = echarts.init(document.getElementById('lineChart_size'));
lineChart_size.setOption(option_size);


$(window).resize(function () {
    lineChart_size.resize();
});